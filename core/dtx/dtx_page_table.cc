...
offset_t DTX::GetPageAddr(PageId id) {

    std::vector<RCQP*> qps;
    auto nodes = global_meta_man->GetAllMemNodes();

    // 创建多线程并行访问所有内存节点的页表
    std::vector<std::future<offset_t>> futures;
            ...
            while (true) {
                ...
                for (int i=0; i<MAX_PAGETABLE_ITEM_NUM_PER_NODE; i++) {
                    if (page_table_node->page_table_items[i].page_id == id && page_table_node->page_table_items[i].valid == true) {
                        // find
                        res = page_table_node->page_table_items[i].frame_id;
                        // Unlock
