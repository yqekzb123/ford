...
MetaManager::MetaManager(std::string bench_name) {
  ...
  RDMA_LOG(INFO) << "All storage meta received";
...
node_id_t MetaManager::GetRemoteStorageMeta(std::string& remote_ip, int remote_port) {
  ...
  StorageMeta meta;
  memcpy(&meta, snooper, sizeof(StorageMeta));
  snooper += sizeof(StorageMeta);
  storage_meta[remote_machine_id] = meta;
  assert(*(uint64_t*)snooper == MEM_STORE_META_END);
  free(recv_buf);
  return remote_machine_id;
}
...
void MetaManager::GetRemoteStorageNodeMR(const RemoteNode& node) {
  // Get remote node's memory region information via TCP
  MemoryAttr remote_storage_mr{};

  while (QP::get_remote_mr(node.ip, node.port, SERVER_STORAGE_ID, &remote_storage_mr) != SUCC) {
    usleep(2000);
  }
  remote_storage_mrs[node.node_id] = remote_storage_mr;
}