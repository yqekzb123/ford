...
bool LocalBatch::ExeBatchRW(coro_yield_t& yield) {
  ...
  double lock_usec, index_usec, read_usec, recalculate_usec, flush_usec, unpin_usec, log_usec, unlock_usec;
  clock_gettime(CLOCK_REALTIME, &tx_start_time);
  #endif
  struct timespec tx_end_time;
  // printf("local_batch.cc:88 start to acquire lock %ld\n", batch_id);
  try {
    // //! 1. 对事务访问的数据项加锁
    // 只读加锁
    ...
    // printf("local_batch.cc:109 acquire lock success %ld\n", batch_id);

    #if OPEN_TIME
    // 计时2
    clock_gettime(CLOCK_REALTIME, &tx_lock_time);
    lock_usec = (tx_lock_time.tv_sec - tx_start_time.tv_sec) * 1000000 + (double)(tx_lock_time.tv_nsec - tx_start_time.tv_nsec) / 1000;
    #endif
    ...
    index_usec = (tx_index_time.tv_sec - tx_lock_time.tv_sec) * 1000000 + (double)(tx_index_time.tv_nsec - tx_lock_time.tv_nsec) / 1000;
    #endif

    //! 3. 读取数据项
    data_list = ReadData(yield, first_dtx);
    // printf("local_batch.cc:131 %ld read data count %ld\n", batch_id, all_keyid.size());
    #if OPEN_TIME
    // 计时4
    clock_gettime(CLOCK_REALTIME, &tx_read_time);
    read_usec = (tx_read_time.tv_sec - tx_index_time.tv_sec) * 1000000 + (double)(tx_read_time.tv_nsec - tx_index_time.tv_nsec) / 1000;
    #endif
  }
  ...
  for (auto item : data_list) {
    ...
    // printf("local_batch.cc:140, thread %ld set the first value of local data table %ld key %ld ptr %p\n",thread_gid, item->table_id, item->key, data_item);
  ...
  recalculate_usec = (tx_recalculate_time.tv_sec - tx_read_time.tv_sec) * 1000000 + (double)(tx_recalculate_time.tv_nsec - tx_read_time.tv_nsec) / 1000;
  #endif
  ...
  flush_usec = (tx_flush_time.tv_sec - tx_recalculate_time.tv_sec) * 1000000 + (double)(tx_flush_time.tv_nsec - tx_recalculate_time.tv_nsec) / 1000;
  #endif
  ...
  unpin_usec = (tx_unpin_time.tv_sec - tx_flush_time.tv_sec) * 1000000 + (double)(tx_unpin_time.tv_nsec - tx_flush_time.tv_nsec) / 1000;
  #endif
  ...
  log_usec = (tx_log_time.tv_sec - tx_unpin_time.tv_sec) * 1000000 + (double)(tx_log_time.tv_nsec - tx_unpin_time.tv_nsec) / 1000;
  #endif
...
FINISH:
  ...
  unlock_usec = (tx_unlock_time.tv_sec - tx_log_time.tv_sec) * 1000000 + (double)(tx_unlock_time.tv_nsec - tx_log_time.tv_nsec) / 1000;
  #endif
  ...
  // 输出计时
  // printf("local_batch.cc:238 1. lock %lf, 2. index %lf, 3. read %lf, 4. recalculate %lf, 5. flush %lf, 6. unpin %lf, 7. log %lf, 8. unlock %lf (us)\n", lock_usec, index_usec, read_usec, recalculate_usec, flush_usec, unpin_usec, log_usec, unlock_usec);
