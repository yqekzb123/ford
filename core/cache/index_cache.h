...
const offset_t INDEX_NOT_FOUND = -1;
...
class IndexCache {
 public:
  ...
  offset_t Search(node_id_t remote_node_id, table_id_t table_id, itemkey_t key) {
    auto node_search = addr_map.find(remote_node_id);
    if (node_search == addr_map.end()) return INDEX_NOT_FOUND;
    auto table_search = node_search->second.find(table_id);
    if (table_search == node_search->second.end()) return INDEX_NOT_FOUND;
    auto offset_search = table_search->second.find(key);
    return offset_search == table_search->second.end() ? INDEX_NOT_FOUND : offset_search->second;
  }
 ...
 private:
  std::unordered_map<node_id_t, std::unordered_map<table_id_t, std::unordered_map<itemkey_t, offset_t>>> addr_map; //三层嵌套的地址缓存，节点、表、数据项
};